...
if isfolder("./Manake Hub Premium script") then
else
	makefolder("./Manake Hub Premium script")
end
local library = {
	Version = "0.36",
	WorkspaceName = "Manake Hub Premium script",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
				...
				if getsynasset then
					...
					elseif (#image > 14) and (string.sub(image, 1, 14) == "synasseturl://") then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Manake Hub Premium script")
							if fold then
							else
								makefolder("./Manake Hub Premium script")
							end
							fold = isfolder("./Manake Hub Premium script/Themes")
							if fold then
							else
								makefolder("./Manake Hub Premium script/Themes")
							end
							fold = isfolder("./Manake Hub Premium script/Themes/SynapseAssetsCache")
							if fold then
							else
								makefolder("./Manake Hub Premium script Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Manake Hub Premium scriptb/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Manake Hub Premium script/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Manake Hub Premium script/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Manake Hub Premium script" then
		library.WorkspaceName = convertfilename(windowName, "Manake Hub Premium script")
	end
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Manake Hub Premium script") then
						makefolder("./Manake Hub Premium script")
					end
					local common_string = "./Manake Hub Premium script/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Manake Hub Premium script/" .. tostring(custom_workspace or library.WorkspaceName)
						end
					...
					local function savestuff(s, get)
						...
						if not get then
							...
							rawfile = string.format("%s/%s.json", common_string, filenameddst)
						end
					...
					local function loadstuff(s, jsonmode, silent)
						...
						if not jsonmode then
							...
							filename = string.format("%s/%s.json", common_string, filenameddst)
						end
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			...
			Workspace = "Manake Hub Premium script Themes",
			Flags = flags,
			Suffix = "Theme",
			Desginer = true
			...
			Name = "Manake Hub Version : " .. tostring(library.Version or "?")
		}}}
